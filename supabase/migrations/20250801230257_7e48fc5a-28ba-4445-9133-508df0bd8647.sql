-- Fix AI usage stats to include admin users
CREATE OR REPLACE FUNCTION public.get_user_ai_usage_stats(start_date_param date DEFAULT (CURRENT_DATE - '30 days'::interval), end_date_param date DEFAULT CURRENT_DATE)
 RETURNS TABLE(user_id uuid, email text, display_name text, daily_interactions integer, total_interactions integer, success_rate numeric, avg_response_time numeric, quota_used integer, quota_limit integer, plan_name text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
BEGIN
  ...
  WHERE p.role IN ('user', 'admin') -- Include both regular users and admin users
