-- Fix the search path security warning for the new function
DROP FUNCTION IF EXISTS reset_user_to_admin_defaults(UUID);
...
BEGIN
  ...
  IF admin_default_id IS NOT NULL THEN
    SELECT EXISTS(
      SELECT 1 FROM public.user_model_access 
    ...
    IF NOT user_has_access THEN
      INSERT INTO public.user_model_access (user_id, provider_id, granted_by, is_enabled, usage_limit, usage_current)
    ...
    INSERT INTO public.user_preferences (user_id, selected_model_id)
